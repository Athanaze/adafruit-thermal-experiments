...
class Adafruit_Thermal:
	...
	def __init__(self, bus=1, baudrate=9600, **kwargs):
		print("Creating a new adafruit thermal")
		...
		self.uart.init(9600, bits=8, parity=None, stop=1, rx=5, tx=4)
	...
	def printBitmapFromFile(self, w, h, filename, ch, LaaT=False):
		print("printych")
		rowBytes = int((w + 7) / 8)  # Round up to next byte boundary
		if rowBytes >= 48:
			rowBytesClipped = 48  # 384 pixels max width
